            ...
            try
            {
                ...
                if (System.IO.File.Exists(path))
                {
                    try
                    {
                        if (dds)
                        {
                            // [CSC] DDS icon loading removed (upstream used the DDSHeaders lib).
                            // CustomScienceContracts only ships PNG icons; see THIRD_PARTY_NOTICES.md.
                            Log.Error("DDS icons are not supported by this build; use PNG/JPG instead.");
                            return false;
                        }
                        else
                        {
